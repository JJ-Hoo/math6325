...
\begin{itemize}
	\item Send an enhanced measurable space $(X,\Omega_X,N_X)$ to the complex *-algebra of bounded morphisms from $X$ to $\C$
	\item Given a morphism $f: X\rightarrow{Y}$, consider the equivalence class of $f$ in $\strictems$.  
\end{itemize}
...
\end{lemma}
\begin{proof}
	Let $f: K\rightarrow{S}$ be a field homomorphism which is not injective.  That is, there exists a nonzero $s\in S$ such that $f(s)=0$.  But then for any $b\in S$ we have: $$f(b) = f(b)f(1)=f(b)f(s\cdot{s^{-1}})=f(b)f(s)f(s^{-1})=0$$, so $f$ is trivial.
\end{proof}
\begin{proposition}
	The only nontrivial pairs in the category \textsf{Field} of fields for which the categorical product exists are those which are isomorphic
\end{proposition}
\begin{proof}
	Imagine there are two nontrivial fields $F_1$ and $F_2$ which have a product $F:=F_1\times F_2$.  Claim: the projection map $\pi_1: F\rightarrow{}F_1$ must be surjective. Let $h: F_1\rightarrow{F}$ be the unique homomorphism into the product $F$ given by $(0,id_{F_1})$, where in this case $0$ is the trivial field homomorphism from $F_1$ to $F_2$.  But by construction $h$ must satisfy:
	$$ \pi_1\circ{h} = id_{F_1}$$.  Now, for any other field $K$ and field homomorphisms $g_1,g_2: F_1\rightarrow{K}$ which satisfy $g_1\circ{\pi_1} = g_2\circ{\pi_2}$, we have: $$g_1\circ{\pi_1}\circ{h} = g_2\circ{\pi_1}\circ{h}$$ $$g_1 = g_2$$, so $\pi_1$ must be an epimorphism, and since morphisms in \textsf{Field} are maps of underlying sets, $\pi_1$ must be surjective (and so too must $\pi_2$, by a symmetric argument).  But then $\pi_1$ and $\pi_2$ are both field homomorphisms which are surjective, so they are in fact both field isomorphisms, which gives $F_1\cong F_2$
\end{proof}
